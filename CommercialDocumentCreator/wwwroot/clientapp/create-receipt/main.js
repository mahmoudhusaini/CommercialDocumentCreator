...
let cancelButton = document.querySelector('#alertCancelButton');
...
createButton.addEventListener('click', (event) => {
    ...
    if (prods.length <= 0) {
        const errorMessage = "Receipt Must Have At Least One Product Item";
...
async function createReceipt(form) {
    ...
    if (response.ok) {
        ...
        commercialDoc.documentNumber = result.receiptNumber;
...
convertSelect.addEventListener('change', (event) => {
    ...
    alertMessage.textContent = "Are You Sure You Want To Convert That Receipt";
    alertBox.style.display = 'flex';
